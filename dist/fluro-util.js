...
.service('DateTools', function(Fluro) {
    ...
    controller.localDate = function(d) {
        ...
        if(Fluro.timezone) {
            ...
            var local = moment.tz(date, Fluro.timezone).format();
            /*
            var localOffset = local.utcOffset();

            
            //Adjust the date
            local.setTime(local.getTime() + localOffset);

            console.log('TIMEZONE OFFSET CHECK', localOffset, Fluro.timezone);
            return local.toDate();
            */
